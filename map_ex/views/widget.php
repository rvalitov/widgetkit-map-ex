...
?>
<div class="{wk}-hidden widgetkit-map-ex" id="<?php echo $map_id; ?>-d" data-uk-check-display></div>
<?php
...
for ($i = 0; $i < sizeof($markers); $i++)
    if (isset($markers[$i]['content'])) {
        echo '<div class="{wk}-hidden" id="' . $markers[$i]['id'] . '">' . $markers[$i]['content'] . '</div>';
...
<script type="widgetkit/mapex" data-id="<?= $map_id ?>" data-class="<?= $settings['class'] ?> {wk}-preserve-width" data-style="width:<?= $width?>;height:<?= $height?>;">
            ...
            if (item) {
                ...
                jQuery('#<?= $map_id ?>-d').on(
                    ...
                    function () {
                        ...
                        var map = self.find('#<?= $map_id ?>');
                        if (!map.length) {
                            MapEx_error('On display.uk.check failed to find map #<?= $map_id ?>');
                            return;
                        }
                        ...
                        if (map.is(':visible')) {
                                ...
                                function () {
                                    ...
                                    if (item) {
                                        ...
                                        if (typeof window['updateMap' + sub_id] === 'function') {
                                            ...
                                            MapEx_info('On display.uk.check event updated map #<?= $map_id ?>');
                                        } else {
                                            MapEx_error('On display.uk.check failed to find updateMap function for map #<?= $map_id ?>');
                                        }
                                    } else {
                                        MapEx_error('On display.uk.check failed to find map #<?= $map_id ?>');
                                    }
                            ...
                            MapEx_info('On display.uk.check map #<?= $map_id ?> skipped, because it\'s not visible');
                        }
                    }
                );
                <?php else: ?>
            ...
            window.functionCheckMapExWidget = function () {
                ...
                $("script").each(function () {
                    ...
                    if ((srcAPI) && (srcAPI.indexOf('/maps.google.com/') > 0)) {
                        MapEx_info('Found Google Maps API loading script: ' + srcAPI);
                        if (countAPILoaded === 0)
                            srcFirstAPI = srcAPI;
                        countAPILoaded++;
                    }
                ...
                switch (countAPILoaded) {
                    case 0:
                        MapEx_error('No script found that loads Google Maps API. It\'s a fatal error. Possible reasons: invalid installation or conflict with other components, modules or plugins.');
                        break;
                    case 1:
                        MapEx_info('Single script detected that loads Google Maps API.');
                        if (srcFirstAPI.length > 0)
                            if (srcFirstAPI.indexOf('wkInitializeGoogleMapsEx') > 0)
                                MapEx_info('The Google Maps API loading script is correct.');
                            else
                                MapEx_error('The Google Maps API loading script is not correct. Some other component, module or plugin on your website overrides our script making this widget inactive.');
                        break;
                    default:
                        if (isUsingMap)
                            MapEx_error('We found out that you are using both MapEx and Map widget on the same page. This leads to conflicts and errors. You can use only MapEx or Map widget, but not both. You can convert all your Map widgets to MapEx widgets in the control panel.');
                        else
                            MapEx_error('Multiple scripts detected that try to load Google Maps API. This may cause unexpected behaviour or malfunction of the widget. Please, turn of other plugins that use Goolge Maps API to fix this error.');
                        break;
                }
            };
